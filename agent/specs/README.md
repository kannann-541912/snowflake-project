...
| Rollback | Deploy any prior `agent_spec.json` via `scripts/deploy_agent.py --spec-version vN` |
...
3. Run evals: `python agent/evals/run_evals.py --spec-version vN`
4. If evals pass, set `status: stable` and open a PR.
5. After merge, CI deploys via `deploy_agent.py --spec-version vN`.
